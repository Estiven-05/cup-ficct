...
class CupController extends Controller
{
    ...
    // Requisitos obligatorios del CUP (se manejan posteriormente en /mis-requisitos)
...
public function rechazarRequisitos(Request $request, $id)
{
    ...
    {
        $totalInscritos = Postulante::count();
        $gruposNecesarios = (int) ceil($totalInscritos / 70);
        $gruposHabilitados = Grupo::count();

        // Cargar grupos con sus relaciones para evitar N+1 queries
        $todosLosGrupos = Grupo::with(['postulantes.notas'])->orderBy('id', 'asc')->get();

        $inconsistencias = $todosLosGrupos->filter(fn($g) => $g->postulantes->count() > 70);

        $gruposConMasPostulantes = $todosLosGrupos->sortByDesc(fn($g) => $g->postulantes->count())->take(5);
        $gruposConMasAprobados = $todosLosGrupos->sortByDesc(fn($g) => $g->postulantes->filter(fn($p) => optional($p->notas)->estado === 'APROBADO')->count())->take(5);

        $grupos = $todosLosGrupos;

        return view('grupos', compact(
            'totalInscritos',
            'gruposNecesarios',
            'gruposHabilitados',
            'inconsistencias',
            'gruposConMasPostulantes',
            'gruposConMasAprobados',
            'grupos'
        ));
    }

    public function recalcularGrupos()
    {
        Grupo::all()->each(function ($grupo) {
            $grupo->update([
                'total_inscritos' => $grupo->postulantes()->count(),
            ]);
        });

        return redirect()->back()->with('success', 'Conteos de grupos recalculados correctamente.');
    }
